...
/*! sends non-personal analytics data to goatcounter.com. @author Blueyescat */
(() => {
	let w = window, d = document, lastClick = {}
	function count(path = location.pathname, isEvent, options = {}) {
		if (location.hostname == "dt.tmp.bz" && !location.port && !(w.callPhantom || w._phantom || w.phantom
			|| w.__nightmare
			|| d.__selenium_unwrapped || d.__webdriver_evaluate || d.__driver_evaluate
			|| navigator.webdriver))
			fetch("https://dt.tmp.bz/a?" + new URLSearchParams({
				p: path,
				r: d.referrer,
