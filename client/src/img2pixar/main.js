...
window.addEventListener("paste", e => {
	...
	trigger(inputFile, "change")
})

window.addEventListener("dragover", e => {
	e.preventDefault()
	mainContainer.style.outline = "2px dashed var(--blue)"
})
window.addEventListener("dragleave", e => {
	if (!e.relatedTarget) mainContainer.style.outline = ""
})
window.addEventListener("drop", e => {
	e.preventDefault()
	mainContainer.style.outline = ""
...
inputWidth.addEventListener("input", function () {
	if (!hasSetHeight) {
		const fit = calcAspectRatioFit(image.width, image.height, this.valueAsNumber, image.height).height
		if (isNaN(fit)) return
...
inputHeight.addEventListener("input", function () {
	if (!hasSetWidth) {
		const fit = calcAspectRatioFit(image.width, image.height, image.width, this.valueAsNumber).width
		if (isNaN(fit)) return
...
buttonProcess.addEventListener("click", async () => {
	...
	const width = cbUseTurretScale.checked ? Math.round(inputWidth.valueAsNumber * 3) : parseInt(inputWidth.value)
	if (!width || width < 1)
		return notice("The entered width is invalid.")
	const height = cbUseTurretScale.checked ? Math.round(inputHeight.valueAsNumber * 3) : parseInt(inputHeight.value)
	if (!height || height < 1)
...
imgResult.addEventListener("click", e => {
	if (resultDragging)
		return
